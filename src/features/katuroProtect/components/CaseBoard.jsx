...
import { AlertTriangle, Clock, Loader2, LayoutList, Archive } from 'lucide-react';
...
export default function CaseBoard({ initialCaseId, onCaseOpened }) {
  ...
  const filtered = cases.filter((c) => (view === 'archived' ? !!c.archived : !c.archived));
      ...
      {loading ? (
        ...
        <div style={{ background: 'var(--kt-card)', borderRadius: 14, border: '1px solid var(--kt-border)', padding: '48px 24px', textAlign: 'center' }}>
          ...
          <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--kt-text-primary)' }}>
            {view === 'archived' ? 'No archived cases' : 'No active cases'}
          </p>
          <p style={{ margin: '6px 0 0', fontSize: 12, color: 'var(--kt-text-secondary)' }}>
            {view === 'archived' ? 'Cases you archive show up here.' : 'Start one from the Intake tab.'}
          </p>
        </div>
      ) : (
